...
public class LuongDAO {
    ...
    public boolean updateLuong(LuongDTO luong) {
        try {
            conn = DBConnection.getConnection();
            stmt = conn.prepareStatement(
                    "UPDATE luong SET LuongCB = ? WHERE MaLuong = ?");
    ...
    public LuongDTO findSalaryByID(String MaLuong) {
        try {
            conn = DBConnection.getConnection();
            stmt = conn.prepareStatement("SELECT * FROM luong WHERE MaLuong = ?");
            stmt.setString(1, MaLuong);

            rs = stmt.executeQuery();

            while (rs.next()) {
                LuongDTO ldto = new LuongDTO();
                ldto.setMaLuong(rs.getString("MaLuong"));
                ldto.setLuongCoBan(rs.getDouble("LuongCB"));
                return ldto;
            }
