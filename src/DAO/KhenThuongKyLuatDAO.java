...
public class KhenThuongKyLuatDAO {
    ...
    public boolean deleteKhenThuongKyLuat(String id) throws AllException{
        try {
            conn = DBConnection.getConnection();
            deleteChiTietKTKLatMaKTKL(id);
            stmt = conn.prepareStatement(
            ...
            throw new AllException("Không thể xóa do có nhân viên đang nhận mã KTKL này");
        } finally {
            DBConnection.closeConnection(conn, stmt);
        }
    }
    ...
    private boolean hasBangLuong() throws SQLException {
        stmt = conn.prepareStatement(
                "SELECT * FROM khenthuongkyluat JOIN chitietluong \n" +
    ...
    private void updateLuongAllNhanVien() throws SQLException {
        String sql = 
                "UPDATE chitietktkl JOIN chitietluong\n" +
    ...
    public KhenThuongKyLuatDTO findKTKLByID(String MaKTKL) {
     ...
     public boolean deleteChiTietKTKLatMaKTKL(String MaKTKL) throws SQLException {
        stmt = conn.prepareStatement(
